...
def post_house_comments(request, housing_id):
  post_house_comment_form = forms.PostHouseCommentForm(request.POST or None) #requestを送信した結果をコメントモデルに挿入するためのform
  house = get_object_or_404(Housing, id=Housing.id)
  comments = HouseComments.objects.fetch_by_housing_id(housing_id) #テーマに対してのコメントを全て取得
  if post_house_comment_form.is_valid():
    ...
    return redirect('house:post_house_comments', housing_id=housing_id)
  return render(
